...
- 문서는 있지만 그걸 어떻게 해석해야 하는지가 없다.
- 좋은 방법을 발견해도 다음에 그걸 쓰지 않는다.

이 문제를 AI로 해결하려면 단순히 "질문에 답하는 챗봇"으로는 부족하다.
...
평가 단계가 독립적인 이유도 이 때문이다 — 자기 결과를 스스로 칭찬하는 루프는 학습이 아니다.
...
한 시스템이 모든 것을 담으면 경계가 무너진다. 역할을 명확히 나눈다:

| 시스템 | 역할 | 이유 |
|---|---|---|
| **Google Drive** | 사람이 만든 원본 문서 | AI가 덮어쓰면 안 되는 진짜 원본 |
| **GBrain + Supabase** | 조직이 학습한 기억 | Drive를 대체하지 않음 — 소화된 지식만 담음 |
...
Extract  → "무엇이 왜 됐고 안 됐는가"를 다음에 쓸 수 있는 Episode로 기록한다
